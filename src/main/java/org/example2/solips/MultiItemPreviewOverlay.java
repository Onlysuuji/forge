...
public class MultiItemPreviewOverlay {
    ...
    public static void onRenderHud(RenderGuiLayerEvent.Post event) {
        ...
        String observationKey =
                ObservedEnchantState.getItem().toString()
                        + "|" + ObservedEnchantState.getBookshelves()
                        + "|" + costs[0] + "," + costs[1] + "," + costs[2]
                        + "|" + clueIds[0] + "," + clueIds[1] + "," + clueIds[2]
                        + "|" + clueLv[0] + "," + clueLv[1] + "," + clueLv[2];

        if (!observationKey.equals(SeedCrackState.getLastObservationKey()) && !SeedCrackState.isRunning()) {
            SeedCrackState.setLastObservationKey(observationKey);
            EnchantSeedCracker.startCrack();
        }

        graphics.drawString(
                font,
                "Observed: " + ObservedEnchantState.getItem().getDescription().getString(),
                left,
                y,
                0xFFFFFF,
                false
        ...
        for (int i = 0; i < 3; i++) {
            graphics.drawString(
                    font,
                    "S" + (i + 1) + ": " + costs[i] + " / id=" + clueIds[i] + " lv=" + clueLv[i],
                    left,
                    y,
                    0xFFFFFF,
                    false
